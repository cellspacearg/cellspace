...
- **Pendiente (fase siguiente):** 31 plantillas ANIMADAS (necesitan infraestructura de video: timeline + canvas animation + export WEBM/MP4). Migrar el catálogo a tablas `design_*` (por ahora el registro vive en el código; funciona, pero para "agregar plantillas sin código" hace falta la arquitectura de datos de la Fase 1).
