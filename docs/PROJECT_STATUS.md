...
**2026-08-09 — Rediseño estilo póster + rubros + colores (según referencia del dueño):**
- `[~]` A partir de una imagen de referencia, se rehízo la placa a un layout tipo póster: cabecera con logo + cinta **HOT SALE** (opcional); producto dentro de un **anillo circular con glow** (acento arriba / azul abajo); **callouts** a la izquierda con líneas al anillo (derivados de `specs`); **lista de 3 features con íconos vectoriales** a la derecha; título; **caja de precio** con borde de acento estilo `USD 380.000 · PRECIO ESPECIAL`; pastilla de transferencia; **fila de beneficios** abajo (entrega / envíos / cuotas) con íconos; footer con **globo (web) + instagram**.
- `[~]` **Rubros** (selector): equipos usados / nuevos / licencias / herramientas / fundas / accesorios / otros. Cada uno define el eyebrow (`// EQUIPO USADO`, `// FUNDA`, …) y su lista de 3 features con íconos propios (batería/escudo/sello/caja/llave/rayo/llave inglesa/funda). Se resuelven con datos reales (batería, garantía).
- `[~]` **Color de acento configurable** (8 temas: naranja/azul/cyan/verde/violeta/rosa/rojo/dorado) — el acento fluye por anillo, caja de precio, íconos, eyebrow y fondos vía `paletteFor()` + `rgba(hex)`. **Moneda ARS/USD** en la caja de precio. Librería de íconos lineales en canvas (`drawIcon`).
- Verificado por captura real (6 variantes + 3 combinaciones rubro/color): layout fiel a la referencia, sin superposiciones. Cache-busting `cb14`. Panel bootea sin errores.
- Nota IA/colores: los 8 temas actúan como paletas sugeridas; una sugerencia de color/copy con IA en vivo requeriría una API key aparte. El asistente puede generar/afinar paletas a pedido en el chat.

**2026-08-09 — Catálogo de 31 plantillas estáticas + textos editables:**
- `[~]` El generador `#/social` pasa a ser un **catálogo de 31 plantillas estáticas** (registro `TEMPLATES`, no hardcode disperso) agrupadas en Producto / Ofertas / Servicio / Software / Institucional. Selector de plantilla en el panel.
- `[~]` **7 archetipos de composición realmente distintos** (no recolores): `hero` (póster referencia), `sale` (headline + burst de % + barra de precio, con fondos rays/diag/cyber/dark), `minimal` (producto centrado con aire), `spec` (ficha técnica), `card` (tarjeta licencia/tool con logo + CTA + circuito), `service` (ícono + título de servicio + features), `editorial` (título grande + producto), `spotlight` (cono de luz). Las 31 plantillas mapean a estos archetipos con parámetros distintos.
- `[~]` **Textos editables** (dejar vacío = automático): título, bajada/subtítulo, precio, % descuento (calcula precio anterior), CTA — vía `effProduct()` que aplica overrides sobre el producto real.
- Funciona en los 3 tamaños + 8 colores + moneda ARS/USD. Verificado por captura (12 plantillas en retrato + 6 combinaciones cuadrado/historia): retrato impecable; se corrigieron choques en ofertas (headline/burst/precio), specs (desborde) y servicio (precio/feature). Pendiente de pulido menor: cuadrado apretado en card/service (precio cerca del footer) y precio anterior en specs-historia. Cache-busting `cb15`. Panel bootea sin errores.
- **Pendiente:** Migrar el catálogo a tablas `design_*` (por ahora el registro vive en el código; funciona, pero para "agregar plantillas sin código" hace falta la arquitectura de datos de la Fase 1).

**2026-08-09 — Plantillas ANIMADAS (motor de motion + export WEBM):**
- `[~]` Modo **Animado** en `#/social`: cada una de las 31 plantillas estáticas se convierte en animada con tratamiento cinematográfico. **7 presets de movimiento**: kenburns (zoom lento), punch (zoom de impacto), float (flotante), slide (entrada deslizada), reveal (cortina), glow (glow + barrido de luz), auto (elige según el archetipo de la plantilla). **Barrido de luz** en todos. **Duración** 15/20/30/45s (default 30).
- `[~]` **Preview en vivo** que reproduce la animación en loop en el panel (requestAnimationFrame). **Exportación a video WEBM** (canvas.captureStream + MediaRecorder, VP9 8Mbps) con indicador de progreso; nombre `producto-tamaño-Ns.webm`. Funciona en Chrome con la pestaña visible.
- Verificado por captura de frames (t=0.02/0.1/0.5/0.95) en punch/reveal/kenburns/float: entrada (fade/scale/wipe/slide) + ambient + barrido correctos. La exportación real (rAF+MediaRecorder) corre en el navegador visible del dueño; no se pudo probar headless (rAF pausado en pestaña oculta). Cache-busting `cb16`. Panel bootea sin errores.
- Nota: MP4 (en vez de WEBM) requeriría `ffmpeg.wasm` (pesado) o render server-side — se puede sumar en una fase avanzada. Para redes, WEBM sirve en la mayoría de los casos; si un canal exige MP4, se convierte el WEBM.
